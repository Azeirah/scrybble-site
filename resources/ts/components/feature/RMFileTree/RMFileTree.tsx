...
export default function RMFileTree() {
  ...
  const {firstTime} = useSyncStatusQuery(undefined, {
    selectFromResult: ({data}) => {
      return {
        firstTime: data.length === 0
      }
    }
  })
  const [showInstallPlugin, setShowInstallPlugin] = useState(firstTime)
  ...
  return <div className="container">
    <Dialog open={showInstallPlugin} close={() => setShowInstallPlugin(false)} title="Since this is your first time">
      <p>
