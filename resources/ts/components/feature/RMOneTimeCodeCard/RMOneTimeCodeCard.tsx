...
export default function RMOneTimeCodeCard({firstTime}: { firstTime: boolean }) {
    ...
    return <div className="card-dark">
        <div className="card-header">
            <span className="fs-4">Connect with ReMarkable {firstTime ?
                <span className="fs-5 text-muted">(step 2/2)</span> : null}</span>
        </div>
        <div className="card-body">
            {!firstTime ? <div className="alert alert-warning">Your authentication token has expired, please log in with
                ReMarkable
                again</div> : null}
            ...
            {firstTime ?
                ...
                :
                <p><strong>Note:</strong> connecting may take <em>well over a minute!</em></p>
            }
            <form onSubmit={(e) => {
                e.preventDefault()
